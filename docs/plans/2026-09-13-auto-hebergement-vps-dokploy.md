...
| A3, étape 5 — Let's Encrypt | — | `traefik.yml` garde l'adresse `test@localhost.com` posée par l'installation ; l'émission réussit quand même (certificat `CN=deploy.takussan.com`, émetteur `YR1`) | laissé tel quel, noté au relevé ; **refermé le 2026-09-14 (TCK-527)** : adresse du compte Dokploy posée, Traefik redémarré, `acme.json` intact — le compte Let's Encrypt déjà enregistré garde son contact d'origine |
