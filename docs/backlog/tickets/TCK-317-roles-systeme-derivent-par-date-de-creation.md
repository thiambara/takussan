...
- [ ] **A — réconcilier au déploiement.** Une commande idempotente
...
- [ ] **B — réconcilier à la lecture.** `AgencyRoleCapabilityCache` complète un rôle
...
- [ ] **C — ne pas matérialiser les rôles système.** `is_system` ne porte pas de lignes
...
- [ ] Quelle que soit l'option : une **garde** qui casse si un rôle système d'une agence
...
- [ ] Écrire la décision en **ADR** (`docs/adr/`) : matérialiser ou non un catalogue
      code-defined est structurel, et c'est la deuxième fois que la question se pose.

## Critères d'acceptation

- [ ] AC1 — Après ajout d'un cas à `Capability`, une agence créée AVANT et une agence
      créée APRÈS accordent les mêmes droits via leur rôle système du même type.
- [ ] AC2 — Un test le prouve en ajoutant réellement une capacité (fixture d'enum ou
      double), pas en supposant le mécanisme.
- [ ] AC3 — Une garde signale toute divergence entre un rôle `is_system` et le
      catalogue, et elle est rejouée en CI.
- [ ] AC4 — Les rôles PERSONNALISÉS ne sont jamais touchés : s'écarter du catalogue est
