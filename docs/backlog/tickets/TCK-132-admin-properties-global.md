...
- **Sparse fields** : `fields[properties]=id,reference_number,title,slug,type,contract_type,status,visibility,price,currency,published_at,created_at`. **Ne pas** y inclure `main_photo_url`, `location` ou `*_label` — ce sont des attributs calculés du `PropertyResource` et spatie rejette avec HTTP 400 (`InvalidFieldQuery`) si on les liste.
