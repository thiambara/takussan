...
- [ ] **Backend** — Inspection : exécuter `POST /api/properties` avec le payload exact envoyé par l'UI (capturé via `Log::debug` temporaire ou `Telescope`) et identifier le champ rejeté
- [ ] **Backend** — Si la cause est une validation enum : aligner les messages d'erreur ou whitelister la valeur attendue
- [ ] **Frontend** — `(dashboard)/app/properties/new/page.tsx` et `(dashboard)/app/properties/[id]/page.tsx` : poster les valeurs canoniques d'enum (pas les labels EN bruts type `Apartment`/`Rent`/`Warehouse` actuellement visibles dans le DOM)
- [ ] **Frontend** — Afficher la traduction FR (« Appartement », « Location », « Entrepôt »…) en label utilisateur uniquement
- [ ] **Frontend** — Supprimer le rendu de l'alerte « Erreur réseau. Réessayez. » au chargement initial sur `/app/properties/[id]` quand la requête succède
- [ ] **Tests backend** — Test feature `agent peut créer un bien via POST /api/properties avec payload UI minimal` (titre + prix + ville + enums)
