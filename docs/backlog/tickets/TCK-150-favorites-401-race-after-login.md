...
- [ ] **Frontend** — Identifier le hook React Query qui appelle `/api/favorites` (probablement `useFavorites` ou similaire dans `takussan-web/src/lib/queries/favorites.ts` / `takussan-web/src/hooks/useFavorite.ts`)
- [ ] **Frontend** — Conditionner la query sur `enabled: !!token` (ou `enabled: status === 'authenticated'` selon l'API du `AuthContext`)
- [ ] **Frontend** — Vérifier qu'aucun autre hook (Header badge, FavoriteButton, etc.) ne tire `/api/favorites` indépendamment sans gate
