# Matrice de corrélation — Passe 005

Légende : ✅ pleinement supporté · ⚠️ partiellement supporté ou capacité latente · ❌ non supporté.

> **Note de stabilité :** ni `docs/features.md` ni `docs/models-spec.md` n'ont été modifiés depuis la passe 001 (commit `57bd3ed`). La matrice est donc strictement identique à celles des passes 001, 002, 003 et 004. Pour éviter la duplication de 250+ lignes inchangées, la passe 005 renvoie à la matrice de référence de la passe 001 : [`pass-001-2026-04-14-0033/01-correlation-matrix.md`](../pass-001-2026-04-14-0033/01-correlation-matrix.md).

---

## 1. Confirmation de non-évolution

Vérification automatique effectuée au début de la passe 005 :

- `git log -1 -- docs/features.md` → `57bd3ed` (identique passes 001–004)
- `git log -1 -- docs/models-spec.md` → `57bd3ed` (identique passes 001–004)
- `sha1sum docs/features.md` → `668257b3391275d49bde5ad3345a50a0dd441405` (identique)
- `sha1sum docs/models-spec.md` → `2fd2dc0ae5727741c0bd9f62f94581adf08aea07` (identique)
- `wc -l docs/features.md` → 399 lignes
- `wc -l docs/models-spec.md` → 1511 lignes

Aucune ligne ajoutée, retirée ou modifiée depuis quatre passes.
...
Les 22 ⚠️ et 9 ❌ sont strictement ceux listés dans la matrice de la passe 001. Aucun item n'a été ajouté ni retiré. Les statuts détaillés par section (§1.1 à §2.9) sont consultables dans le document de référence linké ci-dessus.
...
**Répartition des 22 ⚠️ :** cf. matrice passe 001, sections §1.1 à §2.9.
