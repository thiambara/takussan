...
Deux enums existent dans le code et doivent être ajoutés à la section Enums de models-spec.md :
...
### R4 — Ajouter les contraintes d'unicité

| Table | Colonnes | Type |
|-------|----------|------|
| bank_statements | `reference_number` | unique |

### R5 — Ajouter les index recommandés
...
### R6 — Ajouter BankStatementLineDirection et BankStatementLineMatchStatus dans le tableau d'invariance

`BankStatementLine.matched_payment_type` + `matched_payment_id` sont un morph manuel, comme `AppNotification.referenceable` — même règle d'invariance (Règle 3). À documenter.

### R7 — Conversation.type enum : divergences

L'enum `ConversationType` dans models-spec.md (§Enums) liste `direct, group, booking, lease, property` mais les valeurs dans le modèle Conversation documenté (`models-spec.md §18`) n'en liste aucune. Les valeurs dans le code sont `direct`, `group`, `support`. La spec doit être alignée — soit supprimer `booking, lease, property` de l'enum si non implémentés, soit documenter le delta.

**Recommandation :** Vérifier le code (`app/Models/Enums/ConversationType.php`) et aligner models-spec.md. Si `support` est implémenté, l'ajouter à l'enum spec.
...
| R3 | Ajouter 4 enums (BankStatementSourceFormat, BankStatementStatus, BankStatementLineDirection, BankStatementLineMatchStatus) | Complétude enums |
