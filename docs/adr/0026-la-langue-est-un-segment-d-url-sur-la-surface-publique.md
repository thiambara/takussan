...
`/app`, `/admin`, `/super-admin`, `/auth`, `/onboarding`, `/publish`, `/maintenance`,
`/verification-indisponible`, `/api/**`, `/_next/**`, `/_vercel/**`, les conventions de fichiers de
Next servies sans extension (`/icon`, `/apple-icon`, `/opengraph-image`, `/twitter-image`, suffixe
numérique compris), et tout chemin dont le **dernier** segment porte une extension (`/robots.txt`,
`/sitemap.xml`, `/manifest.webmanifest`, `/favicon.ico`).
...
⚠️ **`/verification-indisponible` a été OUBLIÉE de la première version de cette liste, et l'oubli
est instructif sur la forme même de la décision.** Elle vit dans le groupe de routes
`(dashboard)/` — donc à la **racine** du chemin, les groupes n'ajoutant aucun segment. Absente de
la liste, elle était jugée localisable et redirigée vers `/fr/verification-indisponible`, **qui
n'est pas une route : 404**. Or c'est la page de secours des gardes *fail-closed*
(`src/lib/access/server-guards.ts`), atteinte précisément quand l'API ne répond plus : le
correctif d'une panne avait été remplacé, en silence, par une autre panne — sur le seul chemin qui
existe pour distinguer « je n'ai pas pu demander » de « non ».

⚠️ **Et une troisième surface avait perdu son URL, sans être un répertoire.** Next sert ses
FICHIERS de métadonnées — `src/app/icon.tsx`, `apple-icon.tsx`, `opengraph-image.tsx`,
`twitter-image.tsx` — sur des URL racine **sans extension**. Mesuré sur `next dev`, proxy neutralisé
pour voir le routeur et non la redirection : `/icon` → 200 `image/png`, `/icon1` → 200 (le suffixe
numérique sert `/icon1`, **pas** `/icon/1`), `/opengraph-image` → 200. Redirigés vers `/fr/icon`,
ils rendaient 404 : l'icône du site et l'image de partage social cessaient d'être servies.

Ils sont couverts par une **règle de forme** (`MOTIF_METADONNEES_NEXT`) et non par la liste, parce
que le suffixe numérique est ouvert — aucune liste écrite à la main ne contient `icon1`, `icon2`, …
`manifest.ts`, `sitemap.ts` et `robots.ts` n'y sont **pas**, et c'est mesuré et non déduit : ils
servent `/manifest.webmanifest`, `/sitemap.xml`, `/robots.txt`, donc la règle du dernier segment les
couvre déjà. Les ajouter réserverait des segments qu'aucune route ne sert.

*Cette liste est écrite à la main parce qu'elle DOIT l'être* : le proxy s'exécute sur le runtime
edge, sans système de fichiers à interroger. **Ce qu'on ne peut pas dériver, on en dérive la
vérification** : `src/i18n/__tests__/routing.test.ts` énumère les premiers segments réellement
servis par `src/app` — groupes dépliés — et exige que chacun y figure. Le test qui existait avant
*bouclait sur la liste* : il éprouvait ce qui y était écrit, jamais ce qui aurait dû y être, et
retirer `onboarding` et `publish` laissait 187 tests sur 187 au vert.
...
- ⚠️ **`NEXT_PUBLIC_SITE_URL` doit être posée sur chaque environnement déployé qui n'est pas la
  ...
  | `takussan-web/.env.example` | la clé y est déclarée |
  | [`docs/infra/frontend-deploiement.json`](../infra/frontend-deploiement.json) | la clé y est relevée, `valeur_production_mesuree: null` |

  **Ce qui est gardé, et ce qui ne l'est pas.** La garde est
  [`.github/workflows/front-deploy-map.yml`](../../.github/workflows/front-deploy-map.yml) : elle
  exige que toute `NEXT_PUBLIC_*` **lue par le code** soit *déclarée* dans
  `takussan-web/.env.example` **et** *relevée* dans `docs/infra/frontend-deploiement.json`. Elle
  garde donc que la clé est **déclarée**. **Aucune garde de ce dépôt ne peut vérifier qu'elle est
  réellement POSÉE chez Vercel** — le SSO ferme le tableau de bord, ce que le relevé consigne déjà
  dans son champ `non_mesure`. Poser la variable reste un geste humain, sur lequel la CI est aveugle.

  ⚠️ **Ce tableau a porté une consigne FAUSSE, et elle l'était dans le sens qui rassure.** Il
  demandait la clé dans « `takussan-web/.env.example` **et** `.env.docker` », sous peine que
  `scripts/check-env-parity.mjs` « ne voie rien ». Les deux moitiés sont fausses, chacune vérifiable
  en une commande : `takussan-web/.env.docker` **n'existe pas** (le seul `.env.docker` du dépôt est
  celui de l'API), et `check-env-parity.mjs:31-34` ne lit **que** `takussan-api/`. Un agent qui
  suivait cette ligne créait un fichier inutile **et se croyait couvert par une garde incapable de
  se déclencher**. Pire : le même commit portait déjà la formulation juste dans
  `frontend-deploiement.json` — deux documents en désaccord sur la même question, et c'était le
  document *canonique* qui portait la version rassurante. C'est [J-05](../journal-des-corrections.md#j-05)
  reproduit à l'intérieur d'un seul commit.
...
- **Le rendu statique ne change pas, et il ne change pas parce qu'il n'y en avait aucun.** La
  question se pose légitimement : `src/app/[locale]/` n'expose pas de `generateStaticParams`, ce qui
  empêcherait Next de pré-rendre les trois langues. Mesuré sur le build de ce commit : **147 routes,
  147 `ƒ (Dynamic)`, zéro `○`, zéro `●`** — le build n'imprime même pas les légendes du statique,
  faute d'en avoir une seule à décrire.

  La cause est globale et antérieure : `src/app/layout.tsx`, layout **racine** de toutes les routes,
  fait `await cookies()` (ligne 33) pour hydrater `AuthProvider`. Une API dynamique dans un layout
  fait basculer tout son sous-arbre en rendu à la demande. Ce fichier est **inchangé** par TCK-434
  (`git diff HEAD~1 HEAD -- src/app/layout.tsx` est vide), et l'ancienne page `/properties` ne
  portait aucune directive de rendu.

  L'argument décisif est dans le compte lui-même : les 147 routes dynamiques incluent `/auth/login`,
  `/onboarding/*`, `/publish`, `/super-admin/*` — que ce ticket n'a pas touchées. *Une cause locale
  à `[locale]` ne peut pas expliquer des routes qu'on n'a pas touchées.* `/properties` était donc
  déjà rendue à la demande avant ce commit, et l'est encore après.

  **`generateStaticParams` n'est délibérément pas ajouté** : il serait inerte tant que le layout
  racine lit les cookies, et un mécanisme inerte suggère une capacité que l'application n'a pas.
  Le jour où l'on voudra pré-rendre la surface publique, c'est ce `cookies()` racine qu'il faudra
  traiter d'abord — et `generateStaticParams` viendra avec, pas avant.
...
### 6. Un préfixe n'est pas un segment — trois fois dans le même fichier

`src/proxy.ts` a confondu un préfixe de chaîne et un segment de chemin **trois fois, sous trois
formes**, et les trois se manifestaient par une URL perdue :

| Où | Ce qui était écrit | L'URL qui tombait dans l'écart |
|---|---|---|
| exclusion des fichiers, dans le `matcher` | `.*\..*` (un point n'importe où) | `/properties/villa-2.5-pieces` → 404 |
| exclusion de l'API, dans le `matcher` | `api` | `/apiary`, `/api-docs`, `/apis` → 404 |
| garde d'authentification | `pathname.startsWith('/app')` | `/apple-icon` → 307 vers `/auth/login` |

*Quand deux barrières divergent, la question n'est jamais laquelle a raison, mais **quelle URL tombe
dans l'écart**.* Les trois sont ancrées sur une frontière de segment, et le balayage engendré de
`src/__tests__/proxy.test.ts` éprouve les sosies de **chaque** surface réservée — `/apiary` pour
`api`, `/appartements` pour `app`, `/authentique` pour `auth` —, la famille étant dérivée de
`SEGMENTS_NON_LOCALISES` elle-même : elle grandit donc avec la constante.

Une recherche a été faite pour une quatrième occurrence : `src/lib/navigation/active-path.ts`
portait déjà la bonne forme (`pathname === href || pathname.startsWith(\`${href}/\`)`). Le dépôt
savait la faire ; ce fichier-ci ne la faisait pas.
