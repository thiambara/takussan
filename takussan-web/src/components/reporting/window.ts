 ...
 *
 * **Précondition : des buckets MENSUELS.** C'est ce que les deux appelants demandent
 * (`granularity: 'month'`). Le jour où un appelant passerait au jour ou à la semaine, cette
 * fonction devrait recevoir la granularité — elle ne peut pas la deviner.
 *
 ...
 */
export function fenetrePrecedente(rows: BorneDeSerie[]): { starts_at: string; ends_at: string } | null {
  ...
  // MOIS quand la borne est un premier du mois, ce qu'elle est toujours ici.
  const premier = /^(\d{4})-(\d{2})/.exec(rows[0].starts_at);
  ...
  const finPrecedente = new Date(annee, mois - 1, 0);
  const debutPrecedent = new Date(annee, mois - 1 - rows.length, 1);

  return { starts_at: enDate(debutPrecedent), ends_at: enDate(finPrecedente) };
}
