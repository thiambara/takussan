...
describe('SuperAdminSidebar — focus clavier (TCK-359)', () => {
  ...
  it('n’affiche plus les libellés de groupe en `stone-500` (3,64:1 sur `stone-900`)', () => {
    const { container } = renderSidebar();

    const libelle = screen.getByText('Opérations');
    expect(libelle.className).not.toContain('text-stone-500');
    expect(libelle.className).toContain('text-stone-400');
    expect(container.innerHTML).not.toContain('text-stone-500');
  });
});
