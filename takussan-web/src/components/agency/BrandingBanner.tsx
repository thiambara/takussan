...
export function BrandingBanner({ href = '/admin/agency' }: BrandingBannerProps) {
  ...
  return (
    <aside
      role="region"
      aria-label={t('title')}
      className="flex flex-wrap items-center gap-4 rounded-2xl border border-warning/30 bg-warning/10 p-4 sm:flex-nowrap sm:p-5"
    >
      <span
        aria-hidden="true"
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 text-sm text-warning/80">{t('description')}</p>
      </div>
      <Link
        href={href}
        className={`${buttonVariants({ variant: 'outline' })} shrink-0 border-warning/30 bg-card text-warning hover:bg-warning/15`}
      >
        {t('cta')}
      </Link>
    </aside>
