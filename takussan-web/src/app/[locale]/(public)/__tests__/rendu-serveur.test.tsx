...
vi.mock('@/lib/api', async (importOriginal) => {
  ...
  return {
    ...
    apiFetch: vi.fn(async (path: string) => {
      ...
      // ⚠️ **Le domaine des villes est servi ICI et non par `reponsePour`.** Depuis la fusion du
      // lot, la liste demande AUSSI `/public/properties/cities` pour borner ses facettes
      // (TCK-433) : sans cette branche, les doublures « une seule charge pour toutes les URL »
      // rendaient `villesDuCatalogue()` NULLE, donc `city=Dakar` hors domaine, donc absente du
      // `<h1>` — un rouge qui accusait la page alors qu'il décrivait la doublure.
      // Les cas qui veulent éprouver l'API éteinte lèvent depuis `reponsePour` et ne passent
      // jamais ici : cette branche ne masque donc aucun cas de panne.
      if (path.startsWith('/public/properties/cities')) {
        return { data: [{ value: 'Dakar' }, { value: 'Thiès' }], meta: { truncated: false } };
      }
