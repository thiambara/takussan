 ...
 *
 * Mirror of the Owner proxy at `/api/me/owner-profiles/[id]/kyc/upload`
 * (TCK-257) and the SP proxy (TCK-261). Server actions don't cleanly
 * forward `multipart/form-data` bodies, so the wizard POSTs directly to
 * this Next route, which re-streams the multipart body to the Laravel
...
export async function POST(req: NextRequest, ctx: { params: Params }): Promise<NextResponse> {
  ...
  const upstream = await fetch(`${API_URL}/api/me/agent-profiles/${id}/kyc/upload`, {
    method: 'POST',
    body: req.body,
