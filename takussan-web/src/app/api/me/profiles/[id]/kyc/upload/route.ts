...
/**
 * TCK-261 — multipart proxy for the SP onboarding KYC uploader.
 *
...
export async function POST(req: NextRequest, ctx: { params: Params }): Promise<NextResponse> {
  ...
  const upstream = await fetch(`${API_URL}/api/me/profiles/${id}/kyc/upload`, {
    method: 'POST',
    // Forward the multipart body byte-for-byte. Do NOT set Content-Type —
    // the boundary header in the original request must be preserved.
    body: req.body,
  ...
  // Preserve the upstream JSON shape (including 422 validation envelopes).
