...
import { type DomainesDeFacette, filtresCanoniques } from '@/lib/canonique';
...
export async function titreEtDescription(
  ...
  // `filtresCanoniques` ne faisait que LIRE l'URL ; il reçoit désormais `domaines` (TCK-433).
  //
  //   · le CONTRÔLE DE DOMAINE (`domaines`, en amont) borne l'ESPACE D'URL INDEXABLES : une
  //     valeur hors domaine n'est plus retenue comme facette et l'URL se replie sur la page nue.
  //   · le `t.has()` (ci-dessous) borne la RECHERCHE DE LIBELLÉ : un type RÉEL dont le libellé
  //     manquerait retombe sur le sujet générique — dégradé, jamais faux.
  //
  // Le premier ferme le défaut mesuré ci-après ; le second reste le seul filet si un type
  // légitime entrait au domaine sans entrer au dictionnaire.
  //
  // Mesuré le 2026-08-28 sur `/fr/properties?type=nimportequoi`, AVANT ces deux garde-fous :
  //
