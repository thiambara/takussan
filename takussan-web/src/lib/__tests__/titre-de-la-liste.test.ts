...
type DomainesDeFacette = import('@/lib/canonique').DomainesDeFacette;

// ⚠️ Les domaines sont un ARGUMENT depuis la fusion du lot (TCK-433 + TCK-432) : `villes` est
// fourni ici en dur plutôt que lu du réseau, ce qui garde ces cas hors-ligne. `?type=nimportequoi`
// est désormais écarté DEUX fois — par le domaine, puis par `t.has()` — et le test éprouve ce que
// la page rend, donc la conjonction des deux.
const DOMAINES: DomainesDeFacette = {
  ...domainesStatiques(),
  // La Map va de la clé REPLIÉE vers la forme canonique — c'est ce repli qui fait que
  // `?city=dakar` rend `?city=Dakar` au lieu d'ouvrir une URL indexable par variante de casse.
  villes: new Map([
    ['dakar', 'Dakar'],
    ['thiès', 'Thiès'],
  ]),
...
const titre = (requete: string) =>
  titreEtDescription(new URLSearchParams(requete), DOMAINES);
...
describe('TCK-432 — une valeur de `type` inconnue ne fabrique pas un titre illisible', () => {
  // ⚠️ **L'ATTENDU A CHANGÉ À LA FUSION DU LOT, et c'est le correctif, pas une régression.**
  // Avant TCK-433, `?type=nimportequoi` traversait le filtre de canonicité : la page se disait
  // « filtrée » par un type inconnu et retombait sur le sujet générique FILTRÉ
  // (`meta.propertiesFiltered.subjectAny`). Depuis le contrôle de domaine, une valeur hors
  // domaine n'est plus retenue comme facette : `retenus` est VIDE et la page est la page NUE,
  // avec son propre titre (`meta.properties.title`). Se replier sur la page nue est exactement
  // ce que le ticket demandait — le même geste que le `other {}` du gabarit ICU.
  it('se replie sur la page NUE, pas sur une page « filtrée » par un type inconnu', async () => {
    const { title } = await titre('type=nimportequoi');

    expect(title).toBe(fr.meta.properties.title);
    expect(title).not.toBe(fr.meta.propertiesFiltered.subjectAny);
  });

  // ⚠️ **`t.has()` N'EST EXERCÉ PAR AUCUN CAS DE CE FICHIER, et c'est déclaré plutôt que masqué.**
  // Mesuré à la fusion du lot : les **16** valeurs de `propertyTypeValues` ont un libellé dans les
  // **trois** dictionnaires (`fr`, `en`, `wo` — 0 manquant de chaque côté). Aucune donnée réelle ne
  // peut donc atteindre ce garde-fou : il protège un type qui entrerait au domaine sans entrer au
  // dictionnaire, ce qui n'arrive pas aujourd'hui.
  //
  // Une première version de ce commentaire était un TEST (`type=villa` en `wo`). Il passait — et il
  // passait aussi **`t.has()` retiré**, vérifié par ablation (md5 relevé avant/après). Il ne
  // prouvait rien : `wo.json` porte bien `property.types.villa`. Un test vert qui survit à
  // l'ablation de ce qu'il prétend garder est pire qu'un trou déclaré, parce qu'il occupe la place.
  //
  // L'exercer demanderait d'injecter un dictionnaire amputé — c'est-à-dire une quatrième locale de
  // test. À faire si `t.has()` doit devenir porteur ; en attendant, le retirer ne casserait aucun
  // test, et il faut le savoir avant de le retirer.
  ...
  it('dans les trois langues', async () => {
    for (const locale of ['fr', 'en', 'wo'] as const) {
      ...
      expect(title).toBe((DICTIONNAIRES[locale] as typeof fr).meta.properties.title);
    }
  });
});
