 ...
 *
 * Le propriétaire, lui, est reconnu **sans attendre le réseau** : c'est de loin le cas le plus
...
export function peutContacterLeBien(params: {
  ...
  /** Le propriétaire annoncé par la fiche, quand elle en porte un. */
  readonly proprietaireId: number | null | undefined;
  /** `null` tant que la résolution n'a pas répondu. */
  readonly resolution: Pick<PropertyConversationResolution, 'can_message'> | null;
}): boolean {
  const { utilisateurId, proprietaireId, resolution } = params;

  if (utilisateurId !== null && proprietaireId != null && proprietaireId === utilisateurId) {
    return false;
  }
