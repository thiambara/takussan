  ...
  template: `
      <div class="flex flex-col gap-4">
          ...
          <div class="flex flex-col gap-2">
              ...
              <p-select-button [options]="presets" [ngModel]="selectedPreset()" (ngModelChange)="onPresetChange($event)"
                               [allowEmpty]="false" size="small"/>
          </div>
          <div *ngIf="showMenuModeButton()" class="flex flex-col gap-2">
              <span class="text-sm text-muted-color font-semibold">Menu Mode</span>
              <p-select-button [ngModel]="menuMode()" (ngModelChange)="onMenuModeChange($event)"
...
export class DashboardConfigurator implements OnInit {
  router = inject(Router);

  config: PrimeNG = inject(PrimeNG);

  layoutService: DashboardLayoutService = inject(DashboardLayoutService);
